    ...
    fn fast_ctx_ready(&self, device_instance: u32, ctx_idx: u32) -> bool {
        ...
        match built {
            Some(f) => {
                ...
                // Kernel-owned (owner 0): the channel's pages, mmap cookies for
                // libeclipse_nvkick, never freed by a userspace GEM_CLOSE.
                ...
                if userd_phys != 0 {
                    ...
                    crate::scheme::gem_mmap::register(nv::fast_userd_gem(ctx_idx), page, 4096, 0);
                }
    ...
    fn nouveau_ioctl_dispatch(
        ...
        match nr {
            ...
            nv::NR_GEM_CPU_PREP => {
                ...
                // Kernel-owned fence / GPFIFO / USERD pages (mmap cookies of the
                // direct-submit channel): no GPU work of the caller to wait for.
                if nv::is_fast_mmap_handle(req.handle) {
                    return Ok(0);
                }
                let known = self
            ...
            nv::NR_GEM_CPU_FINI => {
                ...
                if gem
                    .iter()
                    .any(|o| o.handle == req.handle && gem_usable_by(o, owner_pid))
                    || nv::is_fast_mmap_handle(req.handle)
                {
                    Ok(0)
                } else {
                    Err(nv::ENOENT)
