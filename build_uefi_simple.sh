...
# Este script crea una ISO UEFI simple que funciona correctamente
...
mkdir -p /tmp/eclipse-uefi-simple/{efi/boot,boot}
...
cp bootloader-uefi/target/x86_64-unknown-uefi/release/eclipse-bootloader-main.efi /tmp/eclipse-uefi-simple/efi/boot/bootx64.efi

# Copiar kernel Eclipse
cp $CARGO_TARGET_DIR/x86_64-unknown-none/release/eclipse_kernel /tmp/eclipse-uefi-simple/boot/

# 5. Crear archivo de información
echo ""
echo "🔧 Paso 5: Creando archivo de información..."
cat > /tmp/eclipse-uefi-simple/README.txt << 'INFO_EOF'
...
sudo dd if=eclipse-os-uefi-simple.iso of=/dev/sdX bs=4M status=progress

Para probar en QEMU:
qemu-system-x86_64 -cdrom eclipse-os-uefi-simple.iso -m 512M -bios /usr/share/qemu/OVMF.fd
...
ls -la /tmp/eclipse-uefi-simple/
echo ""
echo "📁 efi/boot:"
ls -la /tmp/eclipse-uefi-simple/efi/boot/
echo ""
echo "📁 boot:"
ls -la /tmp/eclipse-uefi-simple/boot/

# 7. Crear imagen ISO UEFI usando xorriso con configuración simple
echo ""
echo "🔧 Paso 7: Creando imagen ISO UEFI con configuración simple..."

xorriso -as mkisofs \
    -o "eclipse-os-uefi-simple.iso" \
    -iso-level 3 \
...
isohybrid --uefi eclipse-os-uefi-simple.iso
...
echo "  - eclipse-os-uefi-simple.iso (Imagen UEFI funcional)"
...
echo "  qemu-system-x86_64 -cdrom eclipse-os-uefi-simple.iso -m 512M -bios /usr/share/qemu/OVMF.fd"
echo ""
echo "💾 Para instalar en USB:"
echo "  sudo dd if=eclipse-os-uefi-simple.iso of=/dev/sdX bs=4M status=progress"
echo ""
