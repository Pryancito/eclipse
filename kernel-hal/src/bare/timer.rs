...
use crate::deadline::{duration_to_ns, is_due, TimerHeap, NO_DEADLINE};
...
fn arm_deadline(now_ns: u64, target_ns: u64) {
    ...
    let Some(span) = crate::deadline::arm_span(
        now_ns,
        target_ns,
        TICK_DUE_NS[cpu].load(Ordering::Relaxed),
        ARMED_NS[cpu].load(Ordering::Relaxed),
    ...
    ARMED_NS[cpu].store(now_ns.saturating_add(span), Ordering::Relaxed);
...
fn rearm_after_tick(now_ns: u64) {
    ...
    let tick_ns = super::arch::timer::fast_tick_ns();
    let tick_due = now_ns.saturating_add(tick_ns);
...
pub fn wall_clock_set(target: Duration) {
    // The offset is unsigned, so the wall clock cannot be set behind the
    // monotonic one: a `settimeofday` to an instant nearer the epoch than
    // this machine's uptime saturates to zero and leaves the clock reading
    // uptime-since-the-epoch. The same conversion, and the same saturation,
    // as every other deadline in this file — see `crate::deadline`.
    let offset = target.saturating_sub(timer_now());
    WALL_CLOCK_OFFSET_NS.store(duration_to_ns(offset), Ordering::Relaxed);
