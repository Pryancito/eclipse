...
impl Syscall<'_> {
    ...
    pub async fn syscall(&mut self, num: u32, args: [usize; 6]) -> isize {
        ...
        if let Err(err) = self.linux_process().seccomp_check(num, &args) {
            return -(err as isize);
        }
        ...
        let ret = match sys_type {
            ...
            // Namespaces: unshare / setns / chroot / pivot_root. Used to be
            // ENOSYS on purpose (so glycin skipped bwrap); they now isolate.
            Sys::UNSHARE => self.sys_unshare(a0),
            Sys::SETNS => self.sys_setns(a0.into(), a1),
            Sys::CHROOT => self.sys_chroot(a0.into()),
            Sys::PIVOT_ROOT => self.sys_pivot_root(a0.into(), a1.into()),
            Sys::SECCOMP => self.sys_seccomp(a0, a1, a2.into()),
            // Newer mount API; bwrap falls back to mount(2) on ENOSYS.
            Sys::OPEN_TREE | Sys::MOVE_MOUNT => Err(LxError::ENOSYS),
