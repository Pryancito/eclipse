...
impl AnimationManager {
        ...
        for id in animation_ids {
            if let Some(animation) = self.animations.get_mut(&id) {
                if animation.state == AnimationState::Running {
                    ...
                    animation.progress =
                        (animation_time as f32 / animation.config.duration as f32).min(1.0);
                    ...
                    let eased_progress = match animation.config.easing {
                        ...
                        EasingType::EaseOut => {
                            1.0 - (1.0 - animation.progress) * (1.0 - animation.progress)
                        }
    ...
    fn apply_easing(&self, t: f32, easing: &EasingType) -> f32 {
        ...
        match easing {
            ...
            EasingType::Circ => 1.0 - self.sqrt(1.0 - t * t),
        }
    }
        ...
        if let Some(animation) = self.animations.get(id) {
            ...
            match &animation.animation_type {
                ...
                AnimationType::Scale {
                    start_scale,
                    end_scale,
                } => Some(AnimationValue::Scale(
                    start_scale + (end_scale - start_scale) * eased_progress,
                )),
                AnimationType::Rotation {
                    start_angle,
                    end_angle,
                } => Some(AnimationValue::Rotation(
                    start_angle + (end_angle - start_angle) * eased_progress,
                )),
                AnimationType::Opacity {
                    start_alpha,
                    end_alpha,
                } => Some(AnimationValue::Opacity(
                    start_alpha + (end_alpha - start_alpha) * eased_progress,
                )),
                AnimationType::Color {
                    start_color,
                    end_color,
                } => Some(AnimationValue::Color(
                    start_color.interpolate(end_color, eased_progress),
                )),
                AnimationType::Size {
                    start_width,
                    start_height,
