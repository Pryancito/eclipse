...
# Este script crea una ISO UEFI que funciona correctamente
...
mkdir -p /tmp/eclipse-uefi-final/{EFI/BOOT,boot}
...
cp bootloader-uefi/target/x86_64-unknown-uefi/release/eclipse-bootloader-main.efi /tmp/eclipse-uefi-final/EFI/BOOT/BOOTX64.EFI

# Copiar kernel Eclipse
cp $CARGO_TARGET_DIR/x86_64-unknown-none/release/eclipse_kernel /tmp/eclipse-uefi-final/boot/

# 5. Crear archivo de información
echo ""
echo "🔧 Paso 5: Creando archivo de información..."
cat > /tmp/eclipse-uefi-final/README.txt << 'INFO_EOF'
...
sudo dd if=eclipse-os-uefi-final.iso of=/dev/sdX bs=4M status=progress

Para probar en QEMU:
qemu-system-x86_64 -cdrom eclipse-os-uefi-final.iso -m 512M -bios /usr/share/qemu/OVMF.fd
...
ls -la /tmp/eclipse-uefi-final/
echo ""
echo "📁 EFI/BOOT:"
ls -la /tmp/eclipse-uefi-final/EFI/BOOT/
echo ""
echo "📁 boot:"
ls -la /tmp/eclipse-uefi-final/boot/
...
xorriso -as mkisofs \
    ...
    -boot-load-size 4 \
    -boot-info-table \
    -output eclipse-os-uefi-final.iso \
    /tmp/eclipse-uefi-final/
...
isohybrid --uefi eclipse-os-uefi-final.iso
...
echo "  - eclipse-os-uefi-final.iso (Imagen UEFI funcional)"
...
echo "  qemu-system-x86_64 -cdrom eclipse-os-uefi-final.iso -m 512M -bios /usr/share/qemu/OVMF.fd"
echo ""
echo "💾 Para instalar en USB:"
echo "  sudo dd if=eclipse-os-uefi-final.iso of=/dev/sdX bs=4M status=progress"
echo ""
