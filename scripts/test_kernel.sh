...
if [ -f "eclipse_kernel/userspace/init/target/x86_64-unknown-linux-musl/release/eclipse-init" ]; then
    test_pass
else
...
INIT_SIZE=$(stat -f%z "eclipse_kernel/userspace/init/target/x86_64-unknown-linux-musl/release/eclipse-init" 2>/dev/null || stat -c%s "eclipse_kernel/userspace/init/target/x86_64-unknown-linux-musl/release/eclipse-init" 2>/dev/null || echo "0")
