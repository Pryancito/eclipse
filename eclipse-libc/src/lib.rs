...
macro_rules! println {
    () => {
        unsafe { $crate::header::stdio::printf("\n\0".as_ptr() as *const $crate::types::c_char); }
    };
    ($fmt:expr) => {
        unsafe { $crate::header::stdio::printf(core::concat!($fmt, "\n\0").as_ptr() as *const $crate::types::c_char); }
    };
    ($fmt:expr, $($arg:tt)*) => {
        unsafe { $crate::header::stdio::printf(core::concat!($fmt, "\n\0").as_ptr() as *const $crate::types::c_char, $($arg)*); }
    };
}
