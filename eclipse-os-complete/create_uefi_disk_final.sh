...
echo "🔧 Creando imagen de disco UEFI final para Eclipse OS"
echo "===================================================="

# Crear imagen de disco de 64MB
echo "💾 Creando imagen de disco..."
dd if=/dev/zero of=eclipse-os-uefi-final.img bs=1M count=64 status=progress

# Crear tabla de particiones GPT usando fdisk
echo "🔧 Configurando partición GPT..."
echo -e "g\nn\n\n\n+50M\nt\n1\nn\n\n\n\nw" | /usr/sbin/fdisk eclipse-os-uefi-final.img

# Crear loop device
echo "🔗 Configurando loop device..."
sudo losetup -fP eclipse-os-uefi-final.img
LOOP_DEV=$(sudo losetup -j eclipse-os-uefi-final.img | cut -d: -f1)
...
sudo dd if=eclipse-os-uefi-final.img of=/dev/sdX bs=4M status=progress

Para probar en QEMU:
qemu-system-x86_64 -drive file=eclipse-os-uefi-final.img,format=raw -m 512M
...
cat > test_uefi_final.sh << 'QEMU_EOF'
#!/bin/bash
...
chmod +x test_uefi_final.sh

echo "✅ Imagen de disco UEFI final creada: eclipse-os-uefi-final.img"
echo "📊 Tamaño del archivo:"
ls -lh eclipse-os-uefi-final.img
echo ""
echo "�� Para probar en QEMU:"
echo "   ./test_uefi_final.sh"
echo ""
echo "💾 Para copiar a USB:"
echo "   sudo dd if=eclipse-os-uefi-final.img of=/dev/sdX bs=4M status=progress"
echo ""
echo "🔍 Verificar particiones:"
echo "   /usr/sbin/fdisk -l eclipse-os-uefi-final.img"
