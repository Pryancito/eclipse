...
impl DisplayDevice {
    ...
    pub fn open() -> Result<Self, DisplayError> {
        #[cfg(target_vendor = "eclipse")]
        {
            ...
            let conn = active_conn.or_else(|| { let f: Option<&control::ConnectorHandle> = conns.first(); f.copied() }).ok_or(DisplayError::OpenFailed)?;
            ...
            let crtc = if conn.0 >= 1000 && conn.0 < 1016 {
                ...
                { let f: Option<&control::CrtcHandle> = crtcs.first(); f.copied() }.ok_or(DisplayError::OpenFailed)?
            };
