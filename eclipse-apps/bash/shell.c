...
get_current_user_info (void)
{
  ...
  if (current_user.user_name == 0)
    {
      /* Eclipse OS: system without users. Hardcode root identity. */
      current_user.user_name = savestring ("root");
      current_user.shell = savestring ("/bin/bash");
      current_user.home_dir = savestring ("/root");
      
      /* Use UID/GID 0 by default if not set */
      current_user.uid = 0;
      current_user.gid = 0;
      current_user.euid = 0;
      current_user.egid = 0;
