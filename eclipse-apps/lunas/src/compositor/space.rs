//! Window space management with stacking, animations, and tiling.
...
impl Space {
    ...
    /// Update window animations using LERP interpolation.
    /// Returns a bitmask of windows that are still animating.
    pub fn update_animations(&mut self, surfaces: &mut [ExternalSurface]) -> u16 {
        ...
        while i < self.window_count {
            ...
            let (tx, ty, tw, th) = if self.windows[i].closing {
                // Closing: shrink to center
                (
                    self.windows[i].curr_x + self.windows[i].curr_w / 2.0,
                    self.windows[i].curr_y + self.windows[i].curr_h / 2.0,
                    0.0,
                    0.0,
                )
            } else if self.windows[i].minimized {
                // Minimized: animate to taskbar position
                let px = (100 + (min_count_anim % 3) * 120) as f32;
                let py = (250 + (min_count_anim / 3) * 150) as f32;
                min_count_anim += 1;
                (px - 20.0, py - 40.0, 40.0, 40.0)
            } else {
                // Normal: animate to target position
                (
                    self.windows[i].x as f32,
                    self.windows[i].y as f32,
                    self.windows[i].w as f32,
                    self.windows[i].h as f32,
                )
            };
...
mod tests {
    ...
    fn test_space_raise_window() {
        let mut space = Space::new();
        let win1 = ShellWindow { x: 1, y: 1, w: 10, h: 10, content: WindowContent::InternalDemo, ..Default::default() };
        let win2 = ShellWindow { x: 2, y: 2, w: 10, h: 10, content: WindowContent::InternalDemo, ..Default::default() };
    ...
    fn test_unmap_invalid_index() {
        ...
        let mut surfaces: [ExternalSurface; MAX_EXTERNAL_SURFACES] =
            core::array::from_fn(|_| ExternalSurface::default());
        space.map_window(ShellWindow {
            w: 100, h: 100, curr_w: 100.0, curr_h: 100.0,
            content: WindowContent::InternalDemo,
            ..Default::default()
        });
    ...
    fn test_map_window_at_capacity() {
        let mut space = Space::new();
        let win = ShellWindow { w: 1, h: 1, curr_w: 1.0, curr_h: 1.0, content: WindowContent::InternalDemo, ..Default::default() };
