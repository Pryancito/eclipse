...
impl TerminalApp {
    fn new() -> Option<Self> {
        ...
        let sh_res = std::fs::read("/bin/bash");
        if sh_res.is_err() { return None; }
        let sh_bytes = sh_res.unwrap();

        let sh_spawn = sys_spawn_with_stdio(&sh_bytes, Some("bash"), pty_slave_fd, pty_slave_fd, pty_slave_fd);
        if sh_spawn.is_err() {
    ...
    fn run(&mut self) {
        ...
        loop {
            ...
            if sys_wait_pid_nohang(&mut status, self.sh_pid as usize).map_or(false, |p| p != 0) {
                ...
                if fd != !0 {
                    if let Ok(pid) = sys_spawn_with_stdio(&self.sh_bytes, Some("bash"), fd, fd, fd) {
                        self.sh_pid = pid;
