...
mod tests {
    ...
    #[test]
    fn test_gl_error_none_is_ok() {
        assert!(GlError::NONE.is_ok());
    }

    #[test]
    fn test_gl_error_invalid_not_ok() {
        assert!(!GlError(GL_INVALID_ENUM).is_ok());
        assert!(!GlError(GL_OUT_OF_MEMORY).is_ok());
    }

    #[test]
    fn test_gl_constants() {
        assert_eq!(GL_NO_ERROR, 0x0000);
