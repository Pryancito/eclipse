...
- **Graphics Stack (DRM/KMS)**: Implementation of the Linux DRM/KMS UAPI that allows running standard graphics software — `Xorg` (`startx`) via the virtual console nodes and the VT/KD `ioctl`s, and Wayland compositors (`wlroots`/`labwc`, using the GPU path by default; `WLR_RENDERER=pixman` remains a manual debugging fallback). Includes PRIME support (dma-buf export/import). See [README-drm.md](README-drm.md) and [README-xorg.md](README-xorg.md).
