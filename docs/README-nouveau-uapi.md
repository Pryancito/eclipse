...
| ioctl | Estado | Notas |
|---|---|---|
| `DRM_IOCTL_NOUVEAU_GETPARAM` | ✅ | `PCI_VENDOR`/`PCI_DEVICE`/`FB_SIZE` reales; `VRAM_BAR_SIZE` es la apertura BAR1 (no la VRAM: NVK compara ambas para decidir su heap host-visible); `CHIPSET_ID` es el **chip id real** de `NV_PMC_BOOT_0` (no el mínimo de la arquitectura: 0x170 decodificaba a GA100/SM80 en vez de GA10x/SM86); `PTIMER_TIME` (=14, no 10 — ver la nota en `nouveau_uapi.rs`) devuelve un contador monótono de ns derivado de CPU (stand-in honesto para `GL_TIMESTAMP`, sin leer BAR0); `EXEC_PUSH_MAX` = 64 (el tope real del `EXEC` de este driver); `GRAPH_UNITS` es **real** (`gpc \| tpc<<8`, con la topología del GSP-RM si el RM está atado; es fatal para la enumeración de Mesa, no puede ser EINVAL); `VRAM_USED` siempre 0 |
...
| `DRM_IOCTL_NOUVEAU_CHANNEL_ALLOC` | 🟡 | El primer proceso RM-backed (el compositor) reusa la escalera `step16`+`step17` (contexto 0); cada **proceso cliente posterior** recibe su PROPIO contexto (`ensure_ctx_for_pid` → `ctx_alloc`, ver «Contexto GPU por proceso»), y solo si eso falla cae a un canal de **descubrimiento** (enumeración OK, `EXEC` ENODEV → software). El respaldo del RM es **por proceso**, no por canal: dos canales del mismo pid son dos nombres para el mismo hardware (NVK crea un contexto de usar-y-tirar solo para leer clases). La tabla de canales se suelta durante el `ensure`/prime (~500 ms de RM) para no bloquear el `EXEC` del compositor. Los campos legacy (`fb_ctxdma_handle`, `subchan[]`) se ignoran: no aplican a Turing+ |
| `DRM_IOCTL_NOUVEAU_CHANNEL_FREE` | 🟡 | Contabilidad: borra la entrada del canal. **NO toca la VM** — en la uAPI nouveau los mapeos de `VM_BIND` pertenecen al FICHERO DRM, no a ningún canal (un drenaje aquí, herencia del modelo de canal único, borraba los binds vivos del primer `vkCreateDevice` cuando el segundo destruía su contexto temporal → MMU fault → `RING FULL`). Los mapeos se reclaman donde toca: `GEM_CLOSE` (por handle) y salida del proceso. Los contextos de cliente (`ctx>=1`) se liberan con `ctx_free` al salir su proceso; si quien libera era el dueño del **ctx 0** RM-backed, también se desmonta el singleton de `step17` (`ctx0_reset`) para que el siguiente `CHANNEL_ALLOC` del compositor reconstruya canal/USERD/GPFIFO en vez de heredar uno colgado |
| `DRM_IOCTL_NOUVEAU_NVIF` | 🟡 | Implementado lo que NVK usa: `NEW` (NV_DEVICE y objetos de clase de motor — en un canal RM-backed alloca el objeto de clase REAL vía `class_alloc`, que es lo que construye el contexto del motor; en canales de descubrimiento, contabilidad), `MTHD(NV_DEVICE_V0_INFO)` (chipset/VRAM reales), `SCLASS` (clases por arquitectura) y `DEL` (libera el objeto RM, con ámbito por pid — el token es un puntero de userspace y puede coincidir entre procesos). Multiplexa cinco tamaños sobre el nr 0x47: solo funciona con despacho por NR |
| `DRM_IOCTL_NOUVEAU_SVM_INIT` / `SVM_BIND` | ❌ | memoria unificada CPU/GPU — fuera de alcance de este hito |
| `DRM_IOCTL_NOUVEAU_VM_INIT` | 🟡 | **Standalone** (NVK lo emite ANTES de `CHANNEL_ALLOC`, durante la creación del physical device; exigir un canal aquí costaba la GPU entera). Acepta y registra el rango `kernel_managed`; la reserva real de VA ocurre en `VM_BIND` contra el RM. Es `_IOW` en la práctica (Mesa lo emite con `drmCommandWrite`) aunque el header diga `_IOWR` — la razón original del despacho por NR |
| `DRM_IOCTL_NOUVEAU_VM_BIND` | 🟡 | **Real**: `eclipse_rm_vm_bind_map`/`unmap` (nuevo en `eclipse_rm_init.c`) generalizan el patrón de `step17` (reservar VA en `hVas` + `Map`) para un handle GEM y dirección elegidos por el caller. **`op_count` real, de 1 a 64** por llamada (`op_ptr` como arreglo de `DrmNouveauVmBindOp`; ver "Huecos conocidos" sobre no-atomicidad entre ops) — 0 o más de 64: `EINVAL`/`EOPNOTSUPP`. **`wait_count == sig_count == 0`** siempre exigido (esperar/señalar aquí no tendría sentido — no hay trabajo de GPU que sincronizar, solo (des)mapeo de VA). **PTE kind**: en Turing+ el kind solo dirige la compresión L2 (el swizzle block-linear lo hace el MOTOR vía métodos de block-height, no la tabla de páginas), y este driver no tiene asignador de comptags. Un kind comprimido que llega **aquí** (VM_BIND) se **mapea sin comprimir** (`0x06`): bytes correctos para una superficie no comprimida, y —crucial— **nunca se rechaza aquí**, porque hacerlo dejaba la superficie SIN mapear y colgaba el canal en el primer draw de NVK que la usaba (`exec_submit_signaled TIMEOUT`, `GPGet` congelado, sin MMU fault — visto en RTX con kind `0x01`). El rechazo temprano de kinds comprimidos se hace en `GEM_NEW` (ver su fila), donde NVK aún puede reaccionar en la asignación; el mapeo-sin-comprimir de VM_BIND es el respaldo anti-cuelgue para los que lleguen solo aquí. Si una superficie dependía de verdad de la compresión se verá corrupta (peor caso), pero el canal sigue y el frame completa. El fix real es soporte de comptags/PLC |
...
- `DrmScheme` gana `nouveau_release_process(pid)` (default: no-op).
  `NvidiaGpu` la implementa con alcance **por proceso** (ya no «un solo
  canal»): drena solo los `VM_BIND` del pid que sale (`owner_pid`),
  libera sus objetos `nouveau_gem` **respetando el refcount PRIME**
  (`dec_ref`, no `unregister` incondicional — un buffer que otro poseedor
  aún importa se mantiene vivo con el dueño desacoplado `owner_pid=0`, y
  lo libera el `GEM_CLOSE` del último), libera sus objetos de clase RM
  (`class_objects_drain_pid`, HIJOS del canal, ANTES que el canal para no
  doble-liberar) y su contexto propio (`ctx_free`, ctx ≥ 1).
