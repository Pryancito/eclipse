...
| `DRM_IOCTL_NOUVEAU_CHANNEL_FREE` | 🟡 | Contabilidad: borra la entrada del canal. **NO toca la VM** — en la uAPI nouveau los mapeos de `VM_BIND` pertenecen al FICHERO DRM, no a ningún canal (un drenaje aquí, herencia del modelo de canal único, borraba los binds vivos del primer `vkCreateDevice` cuando el segundo destruía su contexto temporal → MMU fault → `RING FULL`). Los mapeos se reclaman donde toca: `GEM_CLOSE` (por handle) y salida del proceso. La escalera `step16`/`step17` en sí no se desmonta (idempotente, una por arranque); los contextos de cliente los libera `ctx_free` al salir su proceso |
