...
| `DRM_IOCTL_NOUVEAU_VM_BIND` | 🟡 | **Real**: `eclipse_rm_vm_bind_map`/`unmap` (nuevo en `eclipse_rm_init.c`) generalizan el patrón de `step17` (reservar VA en `hVas` + `Map`) para un handle GEM y dirección elegidos por el caller. **`op_count` real, de 1 a 64** por llamada (`op_ptr` como arreglo de `DrmNouveauVmBindOp`; ver "Huecos conocidos" sobre no-atomicidad entre ops) — 0 o más de 64: `EINVAL`/`EOPNOTSUPP`. **`wait_count == sig_count == 0`** siempre exigido (esperar/señalar aquí no tendría sentido — no hay trabajo de GPU que sincronizar, solo (des)mapeo de VA). **PTE kind**: en Turing+ solo son byte-exactos aquí pitch `0x00` y generic-uncompressed `0x06`; un kind comprimido necesita comptags/PLC reales. Este driver no tiene ese soporte para sus BOs en sysmem, así que ahora los kinds no `{0x00,0x06}` se **rechazan explícitamente** (`EOPNOTSUPP`) en vez de mapearse “como genérico” y dejar una lectura/escritura con layout distinto al que NVK construyó (la causa de la imagen corrupta) |
