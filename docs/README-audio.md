...
- **Processing components** (`drivers/src/audio/pipeline/`), SOF's
  ...
  `src` is a polyphase FIR sample-rate converter after SOF's SRC and
  libsamplerate's sinc converter: one Kaiser-windowed prototype low-pass
  (256 phases, 16 taps, ~-90 dB stopband, generated by
  `tools/audio-src-coef/generate.py`) indexed at a fractional phase per
