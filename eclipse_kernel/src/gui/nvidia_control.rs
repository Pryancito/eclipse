...
use crate::gui::nvidia::{NvidiaFanControl, NvidiaPowerMode};
...
impl NvidiaControlPanel {
    ...
    fn render_overclock(&self, framebuffer: &mut crate::gui::framebuffer::Framebuffer) {
        if let Some(driver) = crate::gui::nvidia::get_nvidia_driver() {
            ...
            render_text(
                framebuffer,
                "El overclock puede dañar tu hardware",
                Point::new(20, y),
                Color::RED,
            );
            y += 15;
            render_text(
                framebuffer,
                "y anular la garantía. Usar bajo",
                Point::new(20, y),
                Color::RED,
            );
            y += 15;
...
pub fn get_nvidia_control_panel() -> Option<&'static mut NvidiaControlPanel> {
    unsafe { NVIDIA_CONTROL_PANEL.as_mut() }
}
