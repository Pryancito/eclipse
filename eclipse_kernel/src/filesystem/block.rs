...
impl BlockDevice {
    ...
    pub fn read_block(&mut self, block_num: u64, buffer: &mut [u8; BLOCK_SIZE]) -> Result<(), &'static str> {
    ...
    pub fn write_block(&mut self, block_num: u64, buffer: &[u8; BLOCK_SIZE]) -> Result<(), &'static str> {
        if block_num >= self.total_blocks {
            return Err("Block number out of range");
        }
...
impl BlockCache {
    ...
    pub fn get_or_load_block(&mut self, block_num: u64) -> Result<&mut [u8; BLOCK_SIZE], &'static str> {
    ...
    fn load_block_from_disk(&mut self, block_num: u64) -> Result<&mut [u8; BLOCK_SIZE], &'static str> {
    ...
    fn write_block_to_disk(&self, block_num: u64, data: &[u8; BLOCK_SIZE]) -> Result<(), &'static str> {
        if let Some(device) = get_block_device() {
