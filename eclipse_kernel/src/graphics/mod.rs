...
use phases::{GraphicsPhase, GraphicsPhaseManager, init_graphics_phase_manager};
...
pub fn get_current_graphics_phase() -> Option<GraphicsPhase> {
    phases::get_graphics_phase_manager()
        .map(|manager| manager.get_state().current_phase)
}
...
pub fn transition_to_drm(framebuffer_info: crate::drivers::framebuffer::FramebufferInfo) -> Result<(), &'static str> {
