        ...
        match url {
            "http://eclipse-os.local/welcome" => {
                Ok(WebPage {
                    ...
                    images: vec![
                        "http://eclipse-os.local/logo.png".to_string(),
                    ],
                })
            },
            "http://eclipse-os.local/about" => {
                Ok(WebPage {
                    url: url.to_string(),
            ...
            "http://eclipse-os.local/features" => {
                Ok(WebPage {
                    url: url.to_string(),
...
<h2>Licencia</h2>
<p>Eclipse OS está licenciado bajo la Licencia MIT.</p>".to_string()
    }
...
<p><a href='http://eclipse-os.local/welcome'>Volver al inicio</a></p>".to_string()
    }
    ...
    pub fn show_history(&self) {
        if self.history.is_empty() {
            ...
            for (i, url) in self.history.iter().enumerate() {
                let marker = if i == self.history_index - 1 { ">" } else { " " };
