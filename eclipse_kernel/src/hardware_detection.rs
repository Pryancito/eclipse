...
impl HardwareDetector {
    ...
    pub fn detect_hardware(&mut self) -> &HardwareDetectionResult {
        ...
        let mut gpus: Vec<GpuInfo> = self.pci_manager.get_gpus()
            .iter()
            .filter_map(|gpu| gpu.clone())
            // Incluir todas las GPUs detectadas (reales o virtuales) de clase display
            .filter(|gpu| gpu.pci_device.class_code == 0x03)
            .collect();
        ...
        let is_virtual = |t: GpuType| matches!(t, GpuType::QemuBochs | GpuType::Virtio | GpuType::Vmware);
        ...
        if !gpus.is_empty() {
            if let Ok(loaded_count) = gpu_driver_manager.load_drivers_for_gpus(&gpus) {
                // Inicializar drivers reales cargados
                if let Ok(initialized_count) = gpu_driver_manager.initialize_all_drivers() {
                    // drivers inicializados
                } else {
                    if let Some(fb) = get_framebuffer() { fb.write_text_kernel("Error inicializando drivers GPU", Color::RED); }
                }
            } else {
                if let Some(fb) = get_framebuffer() { fb.write_text_kernel("Error cargando drivers GPU", Color::RED); }
            }
        } else {
            // sin GPUs para drivers
        }
