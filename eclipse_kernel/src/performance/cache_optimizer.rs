...
impl CacheOptimizer {
    ...
    fn find_in_l1(&mut self, address: u64) -> Option<&mut CacheLine> {
        ...
        self.l1_cache.iter_mut().find(|line| line.address == cache_line_address)
    }

    /// Buscar en L2 cache
    fn find_in_l2(&mut self, address: u64) -> Option<&mut CacheLine> {
        let cache_line_address = self.get_cache_line_address(address);
        self.l2_cache.iter_mut().find(|line| line.address == cache_line_address)
    }

    /// Buscar en L3 cache
    fn find_in_l3(&mut self, address: u64) -> Option<&mut CacheLine> {
        let cache_line_address = self.get_cache_line_address(address);
        self.l3_cache.iter_mut().find(|line| line.address == cache_line_address)
    }
    ...
    fn promote_to_l1(&mut self, cache_line: CacheLine) {
        // Remover de L2
        self.l2_cache.retain(|line| line.address != cache_line.address);
        // Agregar a L1
    ...
    fn promote_to_l2(&mut self, cache_line: CacheLine) {
        // Remover de L3
        self.l3_cache.retain(|line| line.address != cache_line.address);
        // Agregar a L2
    ...
    fn cleanup_caches(&mut self) {
        ...
        self.l1_cache.retain(|line| current_time - line.last_access < cleanup_threshold);
        self.l2_cache.retain(|line| current_time - line.last_access < cleanup_threshold);
        self.l3_cache.retain(|line| current_time - line.last_access < cleanup_threshold);
    }
    ...
    fn calculate_memory_hit_rate(&self) -> f64 {
        ...
        (self.metrics.cache_misses as f64 / (self.metrics.cache_hits + self.metrics.cache_misses) as f64) * 100.0
    }
    ...
    fn update_total_hit_rate(&mut self) {
        ...
        self.metrics.total_hit_rate = (self.metrics.cache_hits as f64 / (self.metrics.cache_hits + self.metrics.cache_misses) as f64) * 100.0;
    }

    /// Actualizar utilización de cache
    fn update_cache_utilization(&mut self) {
        let total_cache_size = self.config.l1_cache_size + self.config.l2_cache_size + self.config.l3_cache_size;
        let used_cache_size = (self.l1_cache.len() + self.l2_cache.len() + self.l3_cache.len()) as u64 * self.config.cache_line_size;
