...
use super::tcp::{TcpSocket, TcpAddress, TcpPort};
use super::udp::{UdpSocket, UdpAddress, UdpPort};
...
impl Socket {
    ...
    pub fn send_to(&mut self, data: &[u8], dest_addr: SocketAddress) -> Result<usize, NetworkError> {
        match self {
            Socket::Tcp(_) => {
                Err(NetworkError::NotSupported)
            },
...
impl SocketManager {
    ...
    /// Crear nuevo socket
    pub fn create_socket(&mut self, socket_type: SocketType, local_addr: SocketAddress) -> Result<u32, NetworkError> {
