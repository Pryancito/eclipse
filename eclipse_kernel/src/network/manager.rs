...
impl NetworkManager {
    ...
    pub fn initialize(&mut self) -> NetworkResult<()> {
        ...
        let mut config = super::interface::InterfaceConfig::new(loopback_ip, IpAddress::new(255, 0, 0, 0));
        ...
        let loopback_route = super::routing::Route::direct(
            IpAddress::loopback(),
            IpAddress::new(255, 0, 0, 0),
            1,
        );
    ...
    pub fn process_ip_packet(&mut self, packet: IpPacket, interface_index: u32) -> NetworkResult<()> {
        if !self.initialized {
            return Err(NetworkError::ProtocolError);
        }
        
        self.stats.ip_stats.increment_received(packet.get_total_length() as u64);
        ...
        match packet.get_protocol() {
            super::ip::IpProtocol::ICMP => {
                ...
                if let Some(icmp_data) = super::icmp::IcmpMessage::from_bytes(&packet.payload) {
                    if let Some(response) = self.icmp.process_message(icmp_data, packet.get_source())? {
    ...
    fn forward_packet(&mut self, packet: IpPacket) -> NetworkResult<()> {
        ...
        if let Some(route) = self.routing.find_best_route(packet.get_destination()) {
            if let Some(interface) = self.interfaces.get_interface(route.get_interface()) {
                ...
                self.stats.ip_stats.increment_sent(packet.get_total_length() as u64);
            }
        } else {
    ...
    /// Enviar respuesta ICMP
    fn send_icmp_response(&mut self, response: super::icmp::IcmpMessage, destination: IpAddress) -> NetworkResult<()> {
    ...
    fn send_icmp_destination_unreachable(&mut self, original_packet: IpPacket) -> NetworkResult<()> {
    ...
    pub fn add_interface(&mut self, name: String, interface_type: InterfaceType, mac_address: MacAddress) -> NetworkResult<u32> {
    ...
    /// Configurar interfaz
    pub fn configure_interface(&mut self, index: u32, ip_address: IpAddress, netmask: IpAddress, gateway: Option<IpAddress>) -> NetworkResult<()> {
        if let Some(interface) = self.interfaces.get_interface_mut(index) {
    ...
    /// Crear socket
    pub fn create_socket(&mut self, socket_type: super::socket::SocketType, local_addr: super::socket::SocketAddress) -> NetworkResult<u32> {
