...
impl InterfaceInfo {
    ...
    pub fn new(name: String, interface_type: InterfaceType, mac_address: MacAddress, index: u32) -> Self {
        Self {
            name,
            interface_type,
            mac_address,
...
impl InterfaceManager {
    ...
    pub fn get_interface(&self, index: u32) -> Option<&NetworkInterface> {
        self.interfaces.iter().find(|iface| iface.info.index == index)
    }
    
    /// Obtener interfaz mutable por índice
    pub fn get_interface_mut(&mut self, index: u32) -> Option<&mut NetworkInterface> {
        self.interfaces.iter_mut().find(|iface| iface.info.index == index)
    }
    ...
    pub fn get_active_interfaces(&self) -> Vec<&NetworkInterface> {
        self.interfaces.iter().filter(|iface| iface.is_active()).collect()
    }
