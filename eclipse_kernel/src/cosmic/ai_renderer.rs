...
use alloc::format;
use super::uuid_system::{SimpleUUID, UUIDGenerator, CounterUUIDGenerator};
...
impl AIRenderer {
    ...
    pub fn generate_object_uuid(&mut self, object_type: ObjectType, content_hash: u64) -> ObjectUUID {
    ...
    fn calculate_content_hash(&self, content: &ObjectContent) -> u64 {
        match content {
            ...
            ObjectContent::Gradient { start_color, end_color, direction } => {
                (*start_color as u64) ^ (*end_color as u64) ^ (*direction as u8 as u64)
            },
            ObjectContent::Border { color, thickness, style } => {
                (*color as u64) ^ (*thickness as u64) ^ (*style as u8 as u64)
            },
            ObjectContent::Shadow { color, offset_x, offset_y, blur } => {
                (*color as u64) ^ (*offset_x as u64) ^ (*offset_y as u64) ^ (*blur as u64)
            },
            ObjectContent::Animation { animation_type, duration, current_frame } => {
    ...
    pub fn update_object_position(&mut self, uuid: &ObjectUUID, x: i32, y: i32) -> Result<(), String> {
    ...
    pub fn update_object_size(&mut self, uuid: &ObjectUUID, width: u32, height: u32) -> Result<(), String> {
        if let Some(object) = self.rendered_objects.get_mut(uuid) {
            object.width = width;
    ...
    pub fn set_object_visibility(&mut self, uuid: &ObjectUUID, visible: bool) -> Result<(), String> {
        if let Some(object) = self.rendered_objects.get_mut(uuid) {
    ...
    pub fn get_visible_objects_by_depth(&self) -> Vec<&RenderedObject> {
        let mut objects: Vec<&RenderedObject> = self.rendered_objects
            .values()
    ...
    pub fn get_detailed_info(&self) -> String {
        ...
        info.push_str(&format!("Objetos totales: {}\n", self.render_stats.total_objects));
        info.push_str(&format!("Objetos visibles: {}\n", self.render_stats.visible_objects));
        info.push_str(&format!("Objetos ocultos: {}\n", self.render_stats.hidden_objects));
        info.push_str(&format!("FPS: {:.1}\n", self.render_stats.render_fps));
        info.push_str(&format!("Tiempo de renderizado: {:.1}ms\n", self.render_stats.render_time));
        info.push_str(&format!("Memoria usada: {} KB\n", self.render_stats.memory_used / 1024));
        info.push_str(&format!("Contador de objetos: {}\n", self.uuid_generator.get_counter()));
        
        // Agrupar por tipo
        let mut type_counts: BTreeMap<ObjectType, u32> = BTreeMap::new();
        for object in self.rendered_objects.values() {
            *type_counts.entry(object.uuid.object_type.clone()).or_insert(0) += 1;
        }
