...
impl WindowOperationsManager {
    ...
    pub fn execute_operation(&mut self, window_id: u32, operation: WindowOperation) -> Result<(), String> {
    ...
    pub fn update_drag(&mut self, current_x: i32, current_y: i32) -> Result<(), String> {
        if let Some(ref drag_state) = self.drag_state {
            ...
            self.execute_operation(drag_state.window_id, WindowOperation::Move { x: new_x, y: new_y })?;
        }
        Ok(())
    }
    ...
    pub fn start_resize(&mut self, window_id: u32, corner: ResizeCorner, start_x: i32, start_y: i32) -> Result<(), String> {
        let window = self.windows.get(&window_id)
            .ok_or_else(|| "Ventana no encontrada".to_string())?;
        ...
        if let Some(resize_state) = self.resize_state.clone() {
            ...
            let (new_width, new_height, new_x, new_y) = match resize_state.resize_corner {
                ...
                ResizeCorner::TopRight => {
                    let width = (resize_state.start_width as i32 + delta_x).max(100) as u32;
                    let height = (resize_state.start_height as i32 - delta_y).max(100) as u32;
                    let y = resize_state.start_y + (resize_state.start_height as i32 - height as i32);
                ...
                ResizeCorner::Right => {
                    let width = (resize_state.start_width as i32 + delta_x).max(100) as u32;
                    (width, resize_state.start_height, resize_state.start_x, resize_state.start_y)
                },
                ResizeCorner::Left => {
                    let width = (resize_state.start_width as i32 - delta_x).max(100) as u32;
                    let x = resize_state.start_x + (resize_state.start_width as i32 - width as i32);
                ...
                ResizeCorner::Bottom => {
                    let height = (resize_state.start_height as i32 + delta_y).max(100) as u32;
                    (resize_state.start_width, height, resize_state.start_x, resize_state.start_y)
                },
                ResizeCorner::Top => {
                    let height = (resize_state.start_height as i32 - delta_y).max(100) as u32;
                    let y = resize_state.start_y + (resize_state.start_height as i32 - height as i32);
            ...
            self.execute_operation(resize_state.window_id, WindowOperation::Resize { width: new_width, height: new_height })?;
            self.execute_operation(resize_state.window_id, WindowOperation::Move { x: new_x, y: new_y })?;
        }
        Ok(())
    }
    ...
    pub fn detect_resize_corner(&self, window_id: u32, x: i32, y: i32) -> Option<ResizeCorner> {
        ...
        let right = x >= window.x + window.width as i32 - resize_margin && x <= window.x + window.width as i32;
        let top = y >= window.y && y <= window.y + resize_margin;
        let bottom = y >= window.y + window.height as i32 - resize_margin && y <= window.y + window.height as i32;
    ...
    pub fn switch_to_next_window(&mut self) {
        ...
        let current_index = windows.iter().position(|&id| id == current_focused).unwrap_or(0);
    ...
    pub fn switch_to_previous_window(&mut self) {
        ...
        let current_focused = self.focused_window.unwrap_or(0);
        let current_index = windows.iter().position(|&id| id == current_focused).unwrap_or(0);
        let prev_index = if current_index == 0 { windows.len() - 1 } else { current_index - 1 };
...
pub fn render_window_controls(fb: &mut FramebufferDriver, window: &WindowInfo) -> Result<(), String> {
    ...
    fb.draw_rect(window.x as u32, window.y as u32, window.width, titlebar_height, titlebar_color);

    // Título de la ventana
    fb.draw_text_simple((window.x + 8) as u32, (window.y + 8) as u32, &window.title, text_color);
    ...
    if window.can_minimize {
        ...
        fb.draw_rect(minimize_x as u32, (window.y + button_margin as i32) as u32, button_size, button_size, minimize_color);
        fb.draw_text_simple((minimize_x + 8) as u32, (window.y + 8) as u32, "−", text_color);
    }
    ...
    if window.can_maximize {
        ...
        fb.draw_rect(maximize_x as u32, (window.y + button_margin as i32) as u32, button_size, button_size, maximize_color);
        let icon = if window.state == WindowState::Maximized { "❐" } else { "□" };
        fb.draw_text_simple((maximize_x + 8) as u32, (window.y + 8) as u32, icon, text_color);
    }

    // Botón cerrar
    let close_x = button_start_x + (button_size + button_margin) as i32 * 2;
    fb.draw_rect(close_x as u32, (window.y + button_margin as i32) as u32, button_size, button_size, close_button_color);
    ...
    fb.draw_rect(window.x as u32, window.y as u32, window.width, window.height, border_color);
