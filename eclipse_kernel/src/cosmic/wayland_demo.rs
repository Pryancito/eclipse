...
use super::wayland_integration::{
    cosmic_protocols::PanelPosition, CosmicWaylandIntegration, NativeAppType,
};
use super::{CosmicEvent, CosmicManager};
...
impl CosmicWaylandDemo {
    ...
    fn create_demo_workspaces(&mut self) -> Result<(), String> {
        ...
        for workspace in workspaces {
            let workspace_id = self
                .cosmic_manager
                .create_virtual_workspace(workspace.clone())?;
    ...
    fn create_demo_applications(&mut self) -> Result<(), String> {
        ...
        let calc_id = self
            .cosmic_manager
            .create_wayland_app(NativeAppType::Calculator)?;
        self.demo_apps.push(calc_id);

        // Crear editor de texto
        let editor_id = self
            .cosmic_manager
            .create_wayland_app(NativeAppType::TextEditor)?;
        self.demo_apps.push(editor_id);

        // Crear gestor de archivos
        let file_manager_id = self
            .cosmic_manager
            .create_wayland_app(NativeAppType::FileManager)?;
        self.demo_apps.push(file_manager_id);

        // Crear configuración
        let settings_id = self
            .cosmic_manager
            .create_wayland_app(NativeAppType::Settings)?;
    ...
    fn configure_demo_panel(&mut self) -> Result<(), String> {
        ...
        self.cosmic_manager
            .configure_wayland_panel(45, PanelPosition::Bottom)?;
        Ok(())
    }
    ...
    fn apply_space_theme(&mut self) -> Result<(), String> {
        self.cosmic_manager
            .change_wayland_theme("deep_space".to_string())?;
        Ok(())
    }
    ...
    fn display_status_info(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        let workspace_text = format!(
            ...
            self.demo_workspaces
                .get((self.current_workspace - 1) as usize)
                .map(|s| s.as_str())
        ...
        if let Ok(stats) = self.cosmic_manager.get_wayland_performance_stats() {
            let perf_text = format!(
                ...
                stats.frame_rate, stats.cpu_usage, stats.gpu_usage, stats.compositor_latency
            );
        ...
        fb.draw_text_simple(
            10,
            120,
            "Tab - Cambiar workspace",
            Color::from_hex(0xcccccc),
        );
        fb.draw_text_simple(
            10,
            140,
            "Escape - Menú de inicio",
            Color::from_hex(0xcccccc),
        );
        fb.draw_text_simple(
            10,
            160,
            "Click izquierdo - Activar aplicación",
            Color::from_hex(0xcccccc),
        );
        fb.draw_text_simple(
            10,
