...
use super::{CosmicManager, CosmicConfig, WindowManagerMode, PerformanceMode};
...
impl CosmicDemo {
    ...
    fn demonstrate_window_creation(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        fb.write_text_kernel(&format!("Total de ventanas: {}", self.demo_windows.len()), Color::CYAN);
    ...
    fn demonstrate_window_management(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        if let Some(&window_id) = self.demo_windows.first() {
            ...
            if self.demo_windows.len() > 1 {
                ...
                fb.write_text_kernel(&format!("Minimizando ventana {}", second_window), Color::CYAN);
            }
        ...
        fb.write_text_kernel(&format!("Ventanas activas: {}", stats.window_count), Color::LIGHT_GRAY);
    ...
    fn demonstrate_ai_features(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        if let Err(e) = self.manager.apply_ai_optimization("reduce_effects") {
            fb.write_text_kernel(&format!("Error aplicando optimización: {}", e), Color::YELLOW);
        } else {
    ...
    fn demonstrate_space_theme(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        if let Some(theme) = &self.manager.theme {
            ...
            fb.write_text_kernel(&format!("Color primario: {}", colors.primary.to_hex()), Color::LIGHT_GRAY);
            fb.write_text_kernel(&format!("Color de fondo: {}", colors.background.to_hex()), Color::LIGHT_GRAY);
            fb.write_text_kernel(&format!("Color de acento: {}", colors.accent.to_hex()), Color::LIGHT_GRAY);
        }
    ...
    fn demonstrate_performance_optimization(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        fb.write_text_kernel(&format!("Memoria: {} MB", stats.memory_usage / (1024 * 1024)), Color::CYAN);
    ...
    pub fn run_main_loop(&mut self, fb: &mut FramebufferDriver) -> Result<(), String> {
        ...
        loop {
            ...
            if frame_count % 60 == 0 {
                let stats = self.manager.get_performance_stats();
                fb.write_text_kernel(&format!("COSMIC: {:.1} FPS, {} ventanas, CPU {:.1}%", 
                    stats.frame_rate, stats.window_count, stats.cpu_usage), Color::LIGHT_GRAY);
            }
