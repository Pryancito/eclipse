...
use super::events::{UsbEventFilter, UsbEventQueue, UsbEventStats};
use super::notifications::{UsbNotificationSystem, UsbSystemNotification};
use super::{
    HotplugConfig, UsbDeviceInfo, UsbDeviceState, UsbDeviceType, UsbHotplugEvent, UsbSpeed,
};
use crate::syslog;
use alloc::boxed::Box;
...
impl UsbHotplugManager {
    ...
    pub fn initialize(&mut self) -> Result<(), String> {
        syslog::log_kernel(
            ...
            "USB_HOTPLUG",
            "Inicializando sistema de hot-plug USB...",
        );

        self.system_notifications
            .add_notification(UsbSystemNotification::SystemInitialized);
        ...
        syslog::log_kernel(
            syslog::SyslogSeverity::Info,
            "USB_HOTPLUG",
            "Sistema de hot-plug USB inicializado correctamente",
        );
        Ok(())
    }
    ...
    fn register_default_callbacks(&mut self) {
        ...
        self.notification_system
            .register_device_connected_callback(connected_callback);
        ...
        let disconnected_callback = Box::new(|device_id: u32| {
            syslog::log_kernel(
                syslog::SyslogSeverity::Info,
                "USB_HOTPLUG",
                &alloc::format!("Dispositivo USB desconectado: ID {}", device_id),
            );
        });
        self.notification_system
            .register_device_disconnected_callback(disconnected_callback);
        ...
        let keyboard_callback = Box::new(|device: &UsbDeviceInfo| {
            syslog::log_kernel(
                syslog::SyslogSeverity::Info,
                "USB_HOTPLUG",
                "Teclado USB detectado",
            );
        });
        self.notification_system
            .register_keyboard_callback(keyboard_callback);
    }
    ...
    pub fn start_polling(&mut self) -> Result<(), String> {
        ...
        self.system_notifications
            .add_notification(UsbSystemNotification::PollingStarted);

        syslog::log_kernel(
            syslog::SyslogSeverity::Info,
            "USB_HOTPLUG",
            "Iniciando polling de dispositivos USB...",
        );
        Ok(())
    }
    ...
    pub fn stop_polling(&mut self) {
        ...
        self.system_notifications
            .add_notification(UsbSystemNotification::PollingStopped);
        syslog::log_kernel(
            syslog::SyslogSeverity::Info,
            "USB_HOTPLUG",
            "Polling de dispositivos USB detenido",
        );
    }
...
impl Drop for UsbHotplugManager {
    fn drop(&mut self) {
        self.stop_polling();
        self.system_notifications
            .add_notification(UsbSystemNotification::SystemShutdown);
    }
}
