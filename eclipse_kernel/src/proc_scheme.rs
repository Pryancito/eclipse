...
use alloc::vec::Vec;
use alloc::format;
use crate::process::{ProcessId, ProcessState, current_process_id};
...
impl Scheme for ProcScheme {
    ...
    fn read(&self, id: usize, buf: &mut [u8], offset: u64) -> Result<usize, usize> {
        ...
        if id == 0x2001 { // meminfo
            ...
            let pid = (id & 0x0FFF) as u32;
            if let Some(p) = crate::process::get_process(pid) {
                let r_lock = p.resources.lock();
                for vma in r_lock.vmas.iter() {
                    let r = if (vma.flags & 1) != 0 { "r" } else { "-" };
                    let w = if (vma.flags & 2) != 0 { "w" } else { "-" };
                    let x = if (vma.flags & 4) != 0 { "x" } else { "-" };
                    let p_mode = if vma.shared_anon_id.is_some() { "s" } else { "p" };
                    let name = if vma.file_backed { " [file]" } else if vma.anon_kernel_slack > 0 { " [slack]" } else { "" };
                    content.extend_from_slice(format!("{:012x}-{:012x} {}{}{}{} 00000000 00:00 0 {}\n", 
                        vma.start, vma.end, r, w, x, p_mode, name).as_bytes());
                }
            } else {
                return Err(ENOENT);
            }
