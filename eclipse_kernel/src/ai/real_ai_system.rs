...
impl RealAISystem {
    ...
    fn generate_real_response(&self, tokens: &[String], patterns: &[String]) -> Result<String, String> {
        if tokens.is_empty() {
        ...
        if patterns.contains(&"sistema".to_string()) {
            return Ok("Información del sistema: Eclipse OS funcionando con hardware real".to_string());
        }
        
        if patterns.contains(&"hardware".to_string()) {
            return Ok("Hardware detectado: GPUs reales, framebuffer activo, drivers funcionales".to_string());
        }
        
        if patterns.contains(&"gráficos".to_string()) {
            return Ok("Sistema de gráficos: Framebuffer real, aceleración por hardware disponible".to_string());
        }
        ...
        Ok(format!("Procesado: {} tokens, {} patrones reconocidos", tokens.len(), patterns.len()))
    }
    ...
    fn learn_from_input(&mut self, tokens: &[String]) {
        ...
        for token in tokens {
            if !self.learned_patterns.contains(token) && token.len() > 2 {
                if self.learned_patterns.len() < 1000 { // Límite de memoria
