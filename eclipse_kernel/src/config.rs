...
impl KernelConfig {
    ...
    pub fn set_metrics_collection_interval(&self, interval_ms: u64) -> Result<(), KernelError> {
        ...
        self.metrics_collection_interval_ms.store(interval_ms, Ordering::SeqCst);
        Ok(())
    }
}
...
impl ConfigManager {
    ...
    fn validate_config_value(&self, item: &ConfigItem, value: &ConfigValue) -> Result<(), KernelError> {
...
pub fn get_config_value(key: &str) -> KernelResult<ConfigValue> {
    let manager = CONFIG_MANAGER.lock().map_err(|_| KernelError::InternalError)?;
    if let Some(ref config_manager) = *manager {
        config_manager.get_config(key).cloned().ok_or(KernelError::ConfigurationNotFound)
    } else {
