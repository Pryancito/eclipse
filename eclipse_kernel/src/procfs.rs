...
use crate::virtual_fs::{FsResult, FilePermissions};
...
pub fn init_procfs() -> FsResult<()> {
    ...
    vfs_lock.create_directory("/proc", FilePermissions::default())?;
    vfs_lock.create_directory("/proc/self", FilePermissions::default())?;
...
";
    ...
    vfs.create_file("/proc/cpuinfo", FilePermissions::default())?;
...
",
    ...
    vfs.create_file("/proc/meminfo", FilePermissions::default())?;
...
fn create_uptime(vfs: &mut crate::virtual_fs::VirtualFileSystem) -> FsResult<()> {
    ...
    vfs.create_file("/proc/uptime", FilePermissions::default())?;
    vfs.write_file("/proc/uptime", uptime.as_bytes())
}
...
pub fn update_process_info(pid: u32) -> FsResult<()> {
    ...
    vfs_lock.create_directory(&pid_dir, FilePermissions::default()).ok();
    
    // Create status file
    let status = create_process_status(pid);
    let status_path = format!("{}/status", pid_dir);
    vfs_lock.create_file(&status_path, FilePermissions::default())?;
    vfs_lock.write_file(&status_path, status.as_bytes())?;
    
    // Create cmdline file
    let cmdline = create_process_cmdline(pid);
    let cmdline_path = format!("{}/cmdline", pid_dir);
    vfs_lock.create_file(&cmdline_path, FilePermissions::default())?;
    vfs_lock.write_file(&cmdline_path, cmdline.as_bytes())?;
    
    // Create stat file
    let stat = create_process_stat(pid);
    let stat_path = format!("{}/stat", pid_dir);
    vfs_lock.create_file(&stat_path, FilePermissions::default())?;
    vfs_lock.write_file(&stat_path, stat.as_bytes())?;
    
    // Create maps file
    let maps = create_process_maps(pid);
    let maps_path = format!("{}/maps", pid_dir);
    vfs_lock.create_file(&maps_path, FilePermissions::default())?;
    vfs_lock.write_file(&maps_path, maps.as_bytes())?;
