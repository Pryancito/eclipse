...
use crate::drivers::resolution_manager::{
    FramebufferInfo as ResolutionFramebufferInfo, ResolutionChangeResult, ResolutionManager,
};
...
impl FramebufferUpdater {
    ...
    pub fn change_resolution(
        ...
        height: u32,
        bits_per_pixel: u32,
    ) -> Result<FramebufferDriver, ResolutionChangeResult> {
                    ...
                    Err(_) => {
                        ...
                        let result =
                            self.resolution_manager
                                .set_mode(width, height, bits_per_pixel);
    ...
    fn change_resolution_uefi_graphics(
        &mut self,
        width: u32,
        height: u32,
        bits_per_pixel: u32,
        ...
        let mut mode_info = self
            .uefi_graphics_manager
            .change_resolution_safe(width, height, bits_per_pixel)
    ...
    fn change_resolution_uefi(
        ...
        let mode_info = self
            .uefi_gop_manager
            .change_resolution(width, height, bits_per_pixel)?;
    ...
    fn create_framebuffer_from_uefi_graphics_mode(
        &self,
        mode_info: &crate::drivers::uefi_graphics::UefiModeInfo,
    ) -> FramebufferDriver {
        ...
        let pixel_bitmask =
            framebuffer.info.red_mask | framebuffer.info.green_mask | framebuffer.info.blue_mask;
    ...
    pub fn change_resolution_by_name(
        &mut self,
        name: &str,
    ) -> Result<FramebufferDriver, ResolutionChangeResult> {
    ...
    fn create_framebuffer_from_info(
        &self,
        fb_info: &ResolutionFramebufferInfo,
    ) -> FramebufferDriver {
        let mut framebuffer = FramebufferDriver::new();
    ...
    pub fn update_kernel_framebuffer(
        &mut self,
        new_fb: FramebufferDriver,
    ) -> Result<(), &'static str> {
    ...
    pub fn is_resolution_supported(&self, width: u32, height: u32, bits_per_pixel: u32) -> bool {
        self.resolution_manager
            .find_mode(width, height, bits_per_pixel)
            .is_some()
    }
    ...
    pub fn get_current_resolution(&self) -> Option<(u32, u32, u32)> {
        self.resolution_manager
            .get_current_mode()
            .map(|mode| (mode.width, mode.height, mode.bits_per_pixel))
    }
    ...
    pub fn apply_resolution_config(
        &mut self,
        config: &ResolutionConfig,
    ) -> Result<FramebufferDriver, ResolutionChangeResult> {
        match config {
            ResolutionConfig::Specific {
                width,
                height,
                bits_per_pixel,
            } => self.change_resolution(*width, *height, *bits_per_pixel),
            ResolutionConfig::ByName { name } => self.change_resolution_by_name(name),
            ResolutionConfig::Highest => self.set_highest_resolution(),
            ResolutionConfig::Default => self.restore_default_resolution(),
        }
    }
}
