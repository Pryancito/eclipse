...
impl NvidiaIntegration {
        ...
        if let Some(vulkan) = &self.vulkan {
            ...
            for device in &vulkan.physical_devices {
                info.push_str(&format!(
                    ...
                    device.device_id,
                    device.name,
                    vulkan.supports_ray_tracing(device.device_id)
                ));
            }
        }
    ...
    pub fn is_fully_available(&self) -> bool {
        self.smi.is_some() && self.cuda.is_some() && self.vulkan.is_some() && self.rtx.is_some()
    }
