...
use crate::drivers::drm_manager::{DrmManager, create_drm_manager};
...
pub enum DrmUserlandResponse {
    ...
    ModeChanged { width: u32, height: u32 },
}
...
impl DrmIntegration {
    ...
    pub fn send_command_to_userland(&mut self, command: DrmKernelCommand) -> Result<DrmUserlandResponse, &'static str> {
        ...
        match command {
            ...
            DrmKernelCommand::SetMode { width, height, refresh_rate } => {
            ...
            DrmKernelCommand::Blit { src_rect: _, dst_rect: _ } => {
    ...
    pub fn execute_integrated_operation(&mut self, command: DrmKernelCommand) -> Result<(), &'static str> {
        // Ejecutar en el kernel
        match command {
            DrmKernelCommand::SetMode { width, height, refresh_rate } => {
                self.kernel_drm_manager.set_mode(width, height, refresh_rate)?;
            },
...
pub fn initialize_drm_integration(framebuffer_info: Option<FramebufferInfo>) -> Result<DrmIntegration, &'static str> {
