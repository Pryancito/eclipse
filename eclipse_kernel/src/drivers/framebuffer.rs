...
impl FramebufferDriver {
    ...
    pub fn write_text_kernel(&mut self, text: &str, color: Color) {
        ...
        for &byte in text.as_bytes() {
            // Si el texto se sale del ancho, hacer wrap a la siguiente línea
            if y + char_height > height {
                // Esperar 3 segundos (3000 ms) usando un bucle de spin para evitar dependencias externas
                // Nota: En un kernel, no se debe usar sleep estándar ni temporizadores de alto nivel
