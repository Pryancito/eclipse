...
impl Color {
    ...
    /// Crear color desde valor hexadecimal (0xRRGGBB)
    ...
    /// Crear color desde valor hexadecimal con alpha (0xAARRGGBB)
...
impl FramebufferDriver {
    ...
    pub fn draw_circle(&mut self, center_x: i32, center_y: i32, radius: u32, color: Color) {
        ...
        while x <= y {
            // Dibujar 8 puntos simétricos del círculo
...
impl FramebufferDriver {
    ...
    fn intel_hardware_fill(&mut self, x: u32, y: u32, width: u32, height: u32, color: Color) -> Result<(), &'static str> {
        ...
        unsafe {
            for y in 0..height {
                let row = self.buffer.offset(((y + y) * pitch + x * bytes_per_pixel as u32) as isize);
    ...
    fn intel_hardware_alpha_blend(&mut self, x: u32, y: u32, width: u32, height: u32, 
        ...
        unsafe {
            for y in 0..height {
                for x_offset in 0..width {
                    let pixel_offset = self.buffer.offset(((y + y) * pitch + (x + x_offset) * bytes_per_pixel as u32) as isize);
...
fn draw_char_scaled(fb: &mut FramebufferDriver, x: u32, y: u32, ch: char, color: Color, scale: u32) {
    let bitmap = match ch {
        'A'..='Z' => get_char_bitmap(ch as u8 - b'A' + 10),
        'a'..='z' => get_char_bitmap(ch as u8 - b'a' + 10),
        '0'..='9' => get_char_bitmap(ch as u8 - b'0'),
        ' ' => [0; 16],
        _ => get_char_bitmap(36),
    };
...
/// Obtener bitmap de un carácter (función auxiliar)
fn get_char_bitmap(index: u8) -> [u8; 16] {
    match index {
        0 => [0x3C, 0x66, 0x66, 0x66, 0x66, 0x66, 0x66, 0x3C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 0
        1 => [0x18, 0x38, 0x18, 0x18, 0x18, 0x18, 0x18, 0x3C, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 1
        2 => [0x3C, 0x66, 0x06, 0x0C, 0x18, 0x30, 0x60, 0x7E, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00], // 2
