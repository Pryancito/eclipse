...
impl GpuController {
    ...
    fn set_bochs_resolution(&self, width: u32, height: u32, bpp: u32) -> Result<(), GpuResult> {
        ...
        crate::syslog::log_kernel(
            ...
            "GPU",
            &alloc::format!(
                "Bochs/QEMU: modo {}x{}x{} solicitado por puertos",
                width,
                height,
                bpp
            ),
    ...
    fn write_gpu_register(&self, offset: u32, value: u32) -> Result<(), &'static str> {
        ...
        let reg_ptr = unsafe { self.virt_mmio_base.offset(offset as isize) as *mut u32 };
    ...
    fn read_gpu_register(&self, offset: u32) -> Result<u32, &'static str> {
        ...
        // Calcular la dirección del registro dentro del área mapeada
        // Se usa como base la dirección virtual mapeada
        let reg_ptr = unsafe { self.virt_mmio_base.offset(offset as isize) as *const u32 };

        // Leer del registro de forma segura (volátil)
        unsafe { Ok(core::ptr::read_volatile(reg_ptr)) }
    }
    ...
    pub fn change_resolution(
        ...
        height: u32,
        bits_per_pixel: u32,
    ) -> Result<FramebufferInfo, GpuResult> {
        if !self.initialized {
        ...
        if let GpuType::QemuBochs = self.gpu_type {
            ...
            let (base, ppsl, pf, r, g, b) = if let Some(fb_arc) =
                crate::drivers::framebuffer_manager::get_global_framebuffer()
            {
                let fb = &*fb_arc;
                (
    ...
    fn set_nvidia_resolution(
        ...
        self.write_gpu_register(0x10, 0x00000000)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Configurar resolución
        self.write_gpu_register(0x14, width)
            .map_err(|_| GpuResult::SetModeFailed)?;
        self.write_gpu_register(0x18, height)
            .map_err(|_| GpuResult::SetModeFailed)?;
        self.write_gpu_register(0x1C, bits_per_pixel)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Calcular stride
        let stride = width * (bits_per_pixel / 8);
        self.write_gpu_register(0x20, stride)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Configurar framebuffer
        let fb_address = self.allocate_framebuffer(width, height, stride)?;
        self.write_gpu_register(0x24, (fb_address & 0xFFFFFFFF) as u32)
            .map_err(|_| GpuResult::SetModeFailed)?;
        self.write_gpu_register(0x28, ((fb_address >> 32) & 0xFFFFFFFF) as u32)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Habilitar video
        self.write_gpu_register(0x10, 0x00000001)
            .map_err(|_| GpuResult::SetModeFailed)?;
    ...
    fn configure_gpu_mode(
        ...
        self.write_gpu_register(0x14, mode_reg)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Configurar bits por píxel
        self.write_gpu_register(0x18, bits_per_pixel)
            .map_err(|_| GpuResult::SetModeFailed)?;
    ...
    fn setup_gpu_framebuffer(
        ...
        self.write_gpu_register(0x20, address as u32)
            .map_err(|_| GpuResult::SetModeFailed)?;
        self.write_gpu_register(0x24, (address >> 32) as u32)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Configurar stride
        self.write_gpu_register(0x28, stride)
            .map_err(|_| GpuResult::SetModeFailed)?;

        // Habilitar aceleración
        self.write_gpu_register(0x30, 0x00000001)
            .map_err(|_| GpuResult::SetModeFailed)?;
