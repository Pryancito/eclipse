...
impl DrmModularDriver {
    ...
    pub fn get_available_modes(&self) -> heapless::Vec<VideoMode, 16> {
        ...
        // Modos comunes
        let common_modes = [
            VideoMode { width: 640, height: 480, refresh_rate: 60, bpp: 32 },
            VideoMode { width: 800, height: 600, refresh_rate: 60, bpp: 32 },
            VideoMode { width: 1024, height: 768, refresh_rate: 60, bpp: 32 },
            VideoMode { width: 1280, height: 720, refresh_rate: 60, bpp: 32 },
            VideoMode { width: 1920, height: 1080, refresh_rate: 60, bpp: 32 },
        ];
    ...
    /// Crear framebuffer
    pub fn create_framebuffer(&mut self, width: u32, height: u32, bpp: u32) -> Result<FramebufferInfo, DriverError> {
        if !self.is_initialized {
            return Err(DriverError::NotAvailable);
        }
    ...
    /// Dibujar rectángulo
    pub fn draw_rectangle(&self, x: u32, y: u32, width: u32, height: u32, color: u32) -> Result<(), DriverError> {
        if !self.is_initialized {
            return Err(DriverError::NotAvailable);
        }
