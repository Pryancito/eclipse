...
impl GpuModularDriver {
    ...
    pub fn set_clock_speed(&mut self, speed: u32) -> Result<(), DriverError> {
        ...
        if speed > 3000 { // Límite razonable
...
impl ModularDriver for GpuModularDriver {
    ...
    fn get_info(&self) -> DriverInfo {
        ...
        match self.gpu_type {
            GpuType::NVIDIA => { let _ = vendor.push_str("NVIDIA Corporation"); },
            GpuType::AMD => { let _ = vendor.push_str("Advanced Micro Devices"); },
            GpuType::Intel => { let _ = vendor.push_str("Intel Corporation"); },
            GpuType::Generic => { let _ = vendor.push_str("Eclipse OS Team"); },
        }
...
pub fn is_gpu_available() -> bool {
    unsafe {
        GPU_MODULAR_DRIVER.is_available()
    }
}
