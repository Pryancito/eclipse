...
impl StorageManager {
    ...
    fn detect_storage_devices_from_polished_pci(&mut self, polished_pci: &crate::drivers::pci_polished::PolishedPciDriver) -> Result<(), &'static str> {
        ...
        for i in 0..device_count {
            ...
            if let Some(device) = polished_pci.get_device(i) {
                ...
                // Detectar controladores de almacenamiento
                // Detectar controladores SATA/IDE
                
                let is_storage_controller = (device.class == 0x01 && (device.subclass == 0x06 || device.subclass == 0x01)) || // SATA/IDE
                ...
                serial_write_str(&format!("STORAGE_MANAGER: is_storage_controller: {}\n", 
                ...
                if base_class == 0x01 || base_class == 0x11 { // Mass storage controller (0x01) o Communication device (0x11)
                    let controller_type = match (base_class, subclass) {
                        ...
                        (0x01, 0x06) => {
                            ...
                            // Detectar controlador Intel SATA
                            if device.vendor_id == 0x8086 && device.device_id == 0x2822 {
                                serial_write_str(&format!("STORAGE_MANAGER: INTEL SATA CONTROLLER detectado\n"));
                                serial_write_str(&format!("STORAGE_MANAGER: VID:{:#x} ({}) DID:{:#x} - Controlador SATA\n", 
                        ...
                        (0x01, 0x04) => {
                            ...
                            let controller_type = if is_sata_raid {
                                StorageControllerType::AHCI // Usar driver AHCI para Intel SATA
                            } else {
    ...
    pub fn read_device_sector_with_type(&self, device_info: &StorageDeviceInfo, sector: u64, buffer: &mut [u8], sector_type: StorageSectorType) -> Result<(), &'static str> {
        ...
        match device_info.controller_type {
            ...
            StorageControllerType::AHCI => {
                ...
                // Usar driver AHCI estándar en lugar de RAID
                serial_write_str("STORAGE_MANAGER: Usando driver AHCI estándar para Intel SATA\n");
                
                // Simular lectura exitosa para testing
                buffer.fill(0);
    ...
    pub fn write_device_sector_real(&self, device_info: &StorageDeviceInfo, sector: u64, buffer: &[u8]) -> Result<(), &'static str> {
        ...
        StorageControllerType::AHCI => {
            ...
            // IntelRaidDriver eliminado - usando lógica simple
            ...
            let base_addr = 0xFEB80000u32; // Dirección base del controlador Intel RAID
            // Simular escritura exitosa
            serial_write_str("STORAGE_MANAGER: Simulando escritura AHCI\n");
            Ok(())
            }
        }
    }
    ...
    pub fn read_device_sector_real(&self, device_info: &StorageDeviceInfo, sector: u64, buffer: &mut [u8]) -> Result<(), &'static str> {
        ...
        StorageControllerType::AHCI => {
            ...
            let base_addr = 0xFEB80000u32; // Dirección típica para Intel RAID
            // Simular lectura exitosa
            serial_write_str("STORAGE_MANAGER: Simulando lectura AHCI\n");
            buffer.fill(0);
            Ok(())
        }
        StorageControllerType::AHCI => {
    ...
    fn analyze_storage_device(&self, device_info: &StorageDeviceInfo) -> (StorageControllerType, u32, &'static str) {
        ...
        if device_info.model.contains("Intel RAID Volume") || device_info.model.contains("RAID Volume") {
            return (StorageControllerType::AHCI, 100, "Intel RAID Volume detectado");
        }
        ...
        if device_info.model.contains("IntelRAID") || device_info.model.contains("Intel RAID") {
            return (StorageControllerType::AHCI, 99, "Intel RAID por nombre detectado");
        }
        ...
        if device_info.model.contains("8086") && 
           (device_info.model.contains("2822") || 
            ...
            return (StorageControllerType::AHCI, 98, "Intel SATA RAID por ID detectado");
        }
    ...
    fn assign_linux_device_names(&mut self) {
        ...
        for device_index in 0..self.devices.len() {
            if controller_types[device_index] == StorageControllerType::AHCI {
        ...
        for device_index in 0..self.devices.len() {
            if controller_types[device_index] != StorageControllerType::AHCI && 
               controller_types[device_index] != StorageControllerType::AHCI {
    ...
    pub fn detect_partitions_linux_style(&mut self) -> Result<(), &'static str> {
        ...
        if processed_devices.is_empty() || !sda_has_valid_partitions {
            ...
            for device_index in 0..device_count {
                ...
                if device_info.controller_type == StorageControllerType::AHCI && !intel_raid_found {
                    processed_devices.push(device_index);
        ...
        for i in 0..4 {
            ...
            if offset + 16 <= mbr_buffer.len() {
                ...
                if let Some(fb) = crate::drivers::framebuffer::get_framebuffer() {
                    ...
                    // Log detallado de los bytes raw de la entrada MBR
                    let raw_bytes = alloc::format!("MBR{}: Raw[{} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}]", 
                                                  i + 1,
                                                  partition_entry[0], partition_entry[1], partition_entry[2], partition_entry[3],
                                                  partition_entry[4], partition_entry[5], partition_entry[6], partition_entry[7],
                                                  partition_entry[8], partition_entry[9], partition_entry[10], partition_entry[11],
                                                  partition_entry[12], partition_entry[13], partition_entry[14], partition_entry[15]);
                    fb.write_text_kernel(&raw_bytes, crate::drivers::framebuffer::Color::CYAN);
                ...
                // Log detallado al serial también
                serial_write_str(&format!("STORAGE_MANAGER: MBR{} - Raw bytes: [{:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X} {:02X}]\n",
                                         i + 1,
                                         partition_entry[0], partition_entry[1], partition_entry[2], partition_entry[3],
                                         partition_entry[4], partition_entry[5], partition_entry[6], partition_entry[7],
                                         partition_entry[8], partition_entry[9], partition_entry[10], partition_entry[11],
