...
impl WaylandClientAPI {
    ...
    fn discover_globals(&mut self) -> Result<(), &'static str> {
        ...
        self.globals.insert("wl_compositor".to_string(), GlobalInfo {
        ...
        self.globals.insert("wl_shell".to_string(), GlobalInfo {
    ...
    pub fn create_buffer(&mut self, surface_id: ObjectId, width: u32, height: u32, format: BufferFormat) -> Result<ObjectId, &'static str> {
        if !self.is_connected.load(Ordering::Acquire) {
            return Err("Not connected to server");
        }
    ...
    /// Crear buffer en shared memory
    fn create_shm_buffer(&mut self, buffer_id: ObjectId, width: u32, height: u32, format: BufferFormat) -> Result<(), &'static str> {
    ...
    pub fn set_window_title(&mut self, shell_surface_id: ObjectId, title: &str) -> Result<(), &'static str> {
        if !self.is_connected.load(Ordering::Acquire) {
            return Err("Not connected to server");
        }
    ...
    pub fn set_app_id(&mut self, shell_surface_id: ObjectId, app_id: &str) -> Result<(), &'static str> {
        if !self.is_connected.load(Ordering::Acquire) {
            return Err("Not connected to server");
        }
    ...
    pub fn set_window_state(&mut self, shell_surface_id: ObjectId, state: ShellSurfaceState) -> Result<(), &'static str> {
        if !self.is_connected.load(Ordering::Acquire) {
            return Err("Not connected to server");
        }
