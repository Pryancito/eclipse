...
impl WaylandServer {
    ...
    fn register_standard_globals(&mut self) -> Result<(), &'static str> {
        ...
        self.register_global("wl_display".to_string(), "wl_display".to_string(), 1)?;

        // wl_compositor global
        self.register_global("wl_compositor".to_string(), "wl_compositor".to_string(), 4)?;

        // wl_shm global (shared memory)
        self.register_global("wl_shm".to_string(), "wl_shm".to_string(), 1)?;

        // wl_output global
        self.register_global("wl_output".to_string(), "wl_output".to_string(), 3)?;

        // wl_seat global (input devices)
        self.register_global("wl_seat".to_string(), "wl_seat".to_string(), 7)?;

        // wl_shell global (ventanas básicas)
        self.register_global("wl_shell".to_string(), "wl_shell".to_string(), 1)?;
    ...
    pub fn run(&mut self) -> Result<(), &'static str> {
        ...
        loop {
            ...
            // Mantener el bucle corriendo para presentar frames continuamente
            // (si se necesita limitar CPU, implementar una pequeña espera)
        }
