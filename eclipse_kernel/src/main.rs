...
pub extern "C" fn uefi_entry() -> ! {
    ...
    // Información de entrada UEFI
    boot_info("UEFI", "Kernel iniciado desde bootloader UEFI");
    boot_info("UEFI", "Transferencia de control exitosa");
...
pub extern "C" fn uefi_entry_with_framebuffer(
    ...
    // Mostrar información del framebuffer recibida
    boot_info("FRAMEBUFFER", &format!("{}x{} @ 0x{:x}", width, height, base_address));
    boot_info("FRAMEBUFFER", &format!("Pixels per scan line: {}", pixels_per_scan_line));
